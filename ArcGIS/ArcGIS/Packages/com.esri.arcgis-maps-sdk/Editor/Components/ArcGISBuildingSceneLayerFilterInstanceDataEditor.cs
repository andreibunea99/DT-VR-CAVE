...
namespace Esri.ArcGISMapsSDK.Editor.Components
{
	...
	public class ArcGISBuildingSceneLayerFilterInstanceDataEditor : PropertyDrawer
	{
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			DrawProperty(position, property, "Building Filters", "IsBuildingAttributeFilterEnabled", OpenFiltersWindow);
		...
		private void OpenFiltersWindow(SerializedProperty property)
		{
			...
			bslFiltersWindow.Init(
				...
				(ArcGISBuildingSceneLayerFilterInstanceData buildingSceneLayerFilterInstanceData) =>
				{
					try
					{
						if (property.FindPropertyRelative("BuildingAttributeFilters").GetValue() == buildingSceneLayerFilterInstanceData.BuildingAttributeFilters && (int)property.FindPropertyRelative("ActiveBuildingAttributeFilterIndex").GetValue() == buildingSceneLayerFilterInstanceData.ActiveBuildingAttributeFilterIndex)
						{
							return;
						}

						property.FindPropertyRelative("BuildingAttributeFilters").SetValue(buildingSceneLayerFilterInstanceData.BuildingAttributeFilters);
						property.FindPropertyRelative("ActiveBuildingAttributeFilterIndex").SetValue(buildingSceneLayerFilterInstanceData.ActiveBuildingAttributeFilterIndex);

						EditorUtility.SetDirty(property.serializedObject.targetObject);
					}
					catch (NullReferenceException)
					{
						layerInstanceData.BuildingSceneLayerFilter.BuildingAttributeFilters = buildingSceneLayerFilterInstanceData.BuildingAttributeFilters;
						layerInstanceData.BuildingSceneLayerFilter.ActiveBuildingAttributeFilterIndex = buildingSceneLayerFilterInstanceData.ActiveBuildingAttributeFilterIndex;
					}
		...
		private List<ArcGISBuildingSceneLayerAttributeStatisticsInstanceData> GetStatisticsFromLayer(ArcGISLayerInstanceData layerInstanceData)
		{
			...
			statisticsFuture.Wait();

			var statistics = statisticsFuture.Get();
		...
		private void OpenDisciplinesCategoriesWindow(SerializedProperty property)
		{
			...
			bslCategoriesWindow.Init(
				...
				(ArcGISBuildingSceneLayerFilterInstanceData buildingSceneLayerFilterInstanceData) =>
				{
					try
					{
						if (property.FindPropertyRelative("EnabledDisciplines").GetValue() == buildingSceneLayerFilterInstanceData.EnabledDisciplines && property.FindPropertyRelative("EnabledCategories").GetValue() == buildingSceneLayerFilterInstanceData.EnabledCategories)
						{
							return;
						}

						property.FindPropertyRelative("EnabledDisciplines").SetValue(buildingSceneLayerFilterInstanceData.EnabledDisciplines);
						property.FindPropertyRelative("EnabledCategories").SetValue(buildingSceneLayerFilterInstanceData.EnabledCategories);

						EditorUtility.SetDirty(property.serializedObject.targetObject);
					}
					catch (NullReferenceException)
					{
						layerInstanceData.BuildingSceneLayerFilter.EnabledDisciplines = buildingSceneLayerFilterInstanceData.EnabledDisciplines;
						layerInstanceData.BuildingSceneLayerFilter.EnabledCategories = buildingSceneLayerFilterInstanceData.EnabledCategories;
					}
