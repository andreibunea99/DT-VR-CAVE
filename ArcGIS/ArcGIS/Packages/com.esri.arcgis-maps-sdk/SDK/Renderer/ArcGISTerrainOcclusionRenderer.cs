...
namespace Esri.ArcGISMapsSDK.Renderer
{
	...
	public class ArcGISTerrainOcclusionRenderer
	{
		...
		private (int, int) ViewSize(Camera camera)
		{
			if (camera == null || camera.targetDisplay < 0 || camera.targetDisplay >= Display.displays.Length)
			{
				return (1, 1); // Default size if the camera or target display is invalid
			}
			else
			{
				return (Math.Max(1, Display.displays[camera.targetDisplay].renderingWidth),
						Math.Max(1, Display.displays[camera.targetDisplay].renderingHeight));
			}
		}
