...
public class Utils
{
    ...
    {
        // Downscale the point coordinates to the heatmap dimenstions
    ...
    static Keypoint TraverseToTargetKeypoint(int edgeId, Keypoint sourceKeypoint, int targetKeypointId, Tensor scores, Tensor offsets, int stride, Tensor displacements)
    {
        ...
        // Get nearest heatmap indices for the source keypoint
        Vector2Int sourceKeypointIndices = GetStridedIndexNearPoint(sourceKeypoint.position, stride, height, width);

        // Retrieve the dsiplacement values for the current indices
        ...
        // Get the offset vector for the displaced keipoint indices
        Vector2 offsetVector = GetOffsetVector(displacedPointIndices.y, displacedPointIndices.x, targetKeypointId, offsets);
